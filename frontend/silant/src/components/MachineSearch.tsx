...
interface MachineData {
  ...
  shipment_date?: string | null; // ISO date string
...
export default function MachineSearch() {
  ...
  const handleSearch = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      console.log('Извлечённый токен:', document.cookie);
      ...
      const response = await fetch(
        '/api/v1/machines/search/',
        {
          ...
          headers: headers,
          credentials: 'include',
          body: JSON.stringify({ factory_number: factoryNumber }),
        }
      );
  ...
  // Вспомогательная функция для отображения полей
  const renderField = (label: string, value: string | null | undefined, isVisible: boolean) => {
    ...
    <div className="machine-search-container">
      ...
      <form onSubmit={handleSearch} className="search-form">
        <div className="form-group">
          <label htmlFor="factoryNumber">Заводской номер машины:</label>
          <input
            type="text"
            id="factoryNumber"
            value={factoryNumber}
      ...
      {machine && (
        <div className="machine-result">
          ...
          {/* Расширенная информация — только для авторизованных */}
          {userStatus === 'authorized' && (
            <div className="section authorized-only">
              <h4>Дополнительная информация (только для авторизованных)</h4>
              <table className="machine-table">
                <tbody>
